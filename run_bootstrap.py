    ...
    for k in range(info['N_ENSEMBLE']):
        ...
        if total_used > 0.0:
            ...
            if 'PRETRAIN' in info['IMPROVEMENT'] and os.path.exists('diayn_net'):
                ...
                aug_obs = torch.Tensor(concat_obs_zs(opy.deepcopy(obs), active_head_one_hot)).to(info['DEVICE'])
                obs = torch.Tensor(obs).to(info['DEVICE'])
                aug_next_obs = torch.Tensor(concat_obs_zs(opy.deepcopy(next_obs), active_head_one_hot)).to(info['DEVICE'])
